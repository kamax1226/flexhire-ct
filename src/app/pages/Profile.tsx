...
import { fetchProfile } from 'utils/graphql/fetchGraphql';
...
export default function Profile() {
  ...
  const verifyBadge = useMemo(() => {
    if (userData) {
      return userData.verified ? (
        ...
        <div className="d-flex unverified-badge">
          <VerifiedUser />
          {' '}
          Unverified
        </div>
  ...
  const subTypesMemo = useMemo(() => userData && userData.profile.freelancer_subtypes.map((subtype: any, id: number) => <SubTypeItem title={subtype.name} key={id} />), [userData]);

  const skillsMemo = useMemo(() => userData && userData.user_skills.map((skill: any, id: number) => <SkillItem key={id} skill={skill.name} exp={skill.experience} />), [userData]);

  const workEducationMemo = useMemo(() => userData && userData.timeline_entries.map((timeline: any, id: number) => (
      ...
      <div className="profile-top-block">
        ...
        <div>
          <h3 className="user-name">
            ...
            -
            {' '}
            {userData.profile.freelancer_type}
            {' '}
          </h3>
          <h3 className="user-experience">
            {userData.profile.total_experience}
            {' '}
            Years Experience
          </h3>
        </div>
